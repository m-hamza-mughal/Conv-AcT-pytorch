...
def accuracy(output, target, topk=(1,)):
    ...
    for k in topk:
        ...
        res.append(correct_k.mul_(100.0 / batch_size).detach().cpu())
    return np.array(res)
...
def test_model(model, data_loader, model_path):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(range(1, len(data_loader)+1), desc="Testing", position=0):
            ...
            per_batch = accs/batch
            if (batch % 100)==0: print(f"Top1 accuracy {per_batch[0]} and Top5 accuracy {per_batch[0]}") 
